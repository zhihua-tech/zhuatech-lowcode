...
@SpringBootTest @AutoConfigureMockMvc class DomainDecisionApiTests {
 ...
 @Test void domainDecisionReturnsAuditableScoreMetricsAndActions() throws Exception {
  mvc.perform(post("/api/domain/decision").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"appCode\":\"APP-SALES-OPS\",\"validationErrors\":0,\"unresolvedDependencies\":0,\"testCoverage\":85,\"criticalSecurityFindings\":0,\"ownerAssigned\":true,\"permissionsReviewed\":true,\"rollbackSnapshotReady\":true}"))
 ...
 @Test void domainRiskScenarioReturnsExpectedBlockingDecision() throws Exception {
  mvc.perform(post("/api/domain/decision").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"appCode\":\"APP-SALES-OPS\",\"validationErrors\":20,\"unresolvedDependencies\":10,\"testCoverage\":10,\"criticalSecurityFindings\":5,\"ownerAssigned\":false,\"permissionsReviewed\":false,\"rollbackSnapshotReady\":false}"))

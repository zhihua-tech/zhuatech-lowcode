...
class LowcodeEnterpriseGovernanceApiTests {
    @Autowired MockMvc mvc;

    @Test
    void batchWorkflowIsAtomicAuditableAndFilterable() throws Exception {
        long first = create("GOV-LOWCODE-001", "gov-lowcode-001", "ZH-BJ", "2026-11");
        long second = create("GOV-LOWCODE-002", "gov-lowcode-002", "ZH-BJ", "2026-11");
    ...
    void fiscalPeriodLockPreventsBusinessMutationUntilAdminUnlocks() throws Exception {
        ...
        mvc.perform(post("/api/enterprise/controls")
                .with(httpBasic("operator", "operator123"))
                .contentType(MediaType.APPLICATION_JSON)
                .content(body("GOV-LOWCODE-LOCKED", "gov-lowcode-locked", "ZH-GZ", "2026-12")))
        ...
        mvc.perform(post("/api/enterprise/controls")
                .with(httpBasic("operator", "operator123"))
                .contentType(MediaType.APPLICATION_JSON)
                .content(body("GOV-LOWCODE-UNLOCKED", "gov-lowcode-unlocked", "ZH-GZ", "2026-12")))
            .andExpect(status().isOk());
    }

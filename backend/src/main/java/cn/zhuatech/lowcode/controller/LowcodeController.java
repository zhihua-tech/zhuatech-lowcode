...
import cn.zhuatech.lowcode.service.LowcodeService;
...
public class LowcodeController {
    private final LowcodeService service; public LowcodeController(LowcodeService service){this.service=service;}
    @GetMapping("/public/about") ApiResponse<Map<String,Object>> about(){return ApiResponse.ok(service.about());}
    @GetMapping("/catalog") ApiResponse<LowcodeService.CatalogView> catalog(){return ApiResponse.ok(service.catalog());}
    @GetMapping("/dashboard") ApiResponse<LowcodeService.Dashboard> dashboard(){return ApiResponse.ok(service.dashboard());}
    ...
    @PostMapping("/records/{id}/comments") ApiResponse<BusinessRecord> comment(@PathVariable Long id,@Valid @RequestBody LowcodeService.CommentRequest request){return ApiResponse.ok(service.comment(id,request));}
    @GetMapping("/sla-summary") ApiResponse<LowcodeService.SlaSummary> sla(){return ApiResponse.ok(service.slaSummary());}
    @PostMapping("/records") ApiResponse<BusinessRecord> create(@Valid @RequestBody LowcodeService.RecordRequest request){return ApiResponse.ok(service.create(request));}
    @PutMapping("/records/{id}") ApiResponse<BusinessRecord> update(@PathVariable Long id,@Valid @RequestBody LowcodeService.RecordRequest request){return ApiResponse.ok(service.update(id,request));}
    @PostMapping("/records/{id}/actions") ApiResponse<BusinessRecord> action(@PathVariable Long id,@Valid @RequestBody LowcodeService.ActionRequest request){return ApiResponse.ok(service.action(id,request));}

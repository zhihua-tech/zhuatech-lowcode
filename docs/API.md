...
| --- | --- | --- |
| `appCode` | String | 应用编码 |
| `validationErrors` | int | 模型校验错误数 |
| `unresolvedDependencies` | int | 未解决依赖数 |
| `testCoverage` | double | 自动化测试覆盖率(%) |
| `criticalSecurityFindings` | int | 严重安全问题数 |
| `ownerAssigned` | boolean | 已指定应用负责人 |
| `permissionsReviewed` | boolean | 权限矩阵已复核 |
| `rollbackSnapshotReady` | boolean | 回滚快照已生成 |
